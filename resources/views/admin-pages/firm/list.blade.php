...
@section('subcontent')
    <div class="intro-y mt-8 flex flex-col items-center sm:flex-row">
        <h2 class="mr-auto text-lg font-medium">Firms</h2>
                                ...
                                <td data-tw-merge class="px-5 py-3 border-b dark:border-darkmode-300">
                                    @php
                                        if ($item) {
                                            ...
                                            $relatedIds = explode(',', $item->gst_text);

                                            // Query the related models based on the IDs
                                            $relatedModels = App\Models\GstAndTextCategory::whereIn('id', $relatedIds)->orderBy('id', 'DESC')->get();
                                        }
                                    @endphp
                                    @foreach ($relatedModels as $ModelsItem)
                                        {{ $ModelsItem->category_name }}
                                    @endforeach
                                </td>
                                <td data-tw-merge class="px-5 py-3 border-b dark:border-darkmode-300">
                                    {{-- <div class="flex items-center justify-center"> --}}
                                    <a class="mr-3 flex items-center" href="{{ route('edit.firm', $item->id) }}">
...
@once
    ...
    <script>
        function goToAddCategoriePage() {
            ...
            window.location.href = "{{ route('add.firm') }}";
        }
    </script>
@endonce
