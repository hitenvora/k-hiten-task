...
@section('subhead')
    <title>purchase Bulk Product</title>
@endsection

@section('subcontent')
    <div class="intro-y mt-8 flex items-center">
        <h2 class="mr-auto text-lg font-medium">purchase Bulk Product</h2>
    </div>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form class="customers-validate-form" action="{{ route('update.perch.order', $Order->id) }}" method="POST"
                                    ...
                                    <div class="block w-full overflow-auto scrolling-touch">
                                        <table
                                            ...
                                            <tbody class="quote-item-container table-body">
                                                @if ($Order->GetAdminCart->count() != 0)
                                                    @foreach ($Order->GetAdminCart as $item)
                                                        <tr class="border-b border-bottom-dashed tax-tr">
                                                                ...
                                                                <select data-tw-merge aria-label=".form-select-lg example"
                                                                    ...
                                                                    @foreach ($Product as $data)
                                                                        <option value="{{ $data->id }}"
                                                                            @if ($item->product_id == $data->id) selected @endif>
                                                                            {{ $data->product_name }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            @endphp
                                                            <td>
                                                                <input class="form-control gst" name="gst[]"
                                                                    type="hidden" value="{{ $taxes }}"
                                                                    onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))"
                                                                    min="0"
                                                                    onkeypress="if(this.value.length==8) return false;"
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                <select aria-label=".form-select-lg example" id="payment_type"
                                                    ...
                                                    name="payment_type" style="margin: 0;">
                                                    <option value="">Please Select Payment Type </option>
                                                    <option value="0"
                                                        @if ($Order->payment_type == '0') selected @endif>Cash</option>
                                                    <option value="1"
                                                        @if ($Order->payment_type == '1') selected @endif>Check</option>
                                                    <option value="2"
                                                        @if ($Order->payment_type == '2') selected @endif>RTGS</option>
                                                </select>
                                                <div class="check_number"
                                                    @if ($Order->payment_type != '1') style="display: none;" @endif>
                                                    <label for="discount" class="form-label mb-1">Check Number</label>
                                                    <div class="relative flex items-stretch w-full">
                                                        <x-base.form-input class="form-control" name="check_number"
                                                            id="check_number" type="text" placeholder="Check Number"
                                                            onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))"
                                                            value="{{ $Order->check_number }}"
                                                            pattern="^\d*(\.\d{0,2})?$" minlength="1" required />
                                                    </div>
                                                </div>
                                                <div class="rtgs_number"
                                                    @if ($Order->payment_type != '2') style="display: none;" @endif>
                                                    <label for="discount" class="form-label mb-1">RTGS Number</label>
                                                    <div class="relative flex items-stretch w-full">
                                                        <x-base.form-input class="form-control" name="rtgs_number"
                                                            id="rtgs_number" type="text" placeholder="RTGS Number"
                                                            onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))"
                                                            value="{{ $Order->rtgs_number }}" pattern="^\d*(\.\d{0,2})?$"
                                                            m inlength="1" required />
                                                    </div>
                                                </div>
                                                <div class="bank_id"
                                                    @if ($Order->payment_type != '1' && $Order->payment_type != '2') style="display: none;" @endif>
                                                    {{-- <div
                                                        class="mb-2 xl:w-1/2 pr-4 pl-4 lg:w-4/5 pr-4 pl-4 sm:w-full pr-4 pl-4 float-left"> --}} <label for="bank_id" class="form-label mb-1">
                                                        Bank Name</label>
                                                    <select aria-label=".form-select-lg example" id="bank_id"
                                                        ...
                                                        @foreach ($bank_list as $item)
                                                            <option value="{{ $item->id }}"
                                                                {{ $Order->bank_id == $item->id ? 'selected' : '' }}>
                                                                {{ $item->bank_name }}
                                                            </option>
                                                        @endforeach
                                            ...
                                            {{-- <div
                                                ...
                                                <div class="relative flex items-stretch w-full">
                                                    <x-base.form-input class="form-control" id="validation-form-1"
                                                        ...
                                                        min="0" value="{{ $Order->discount_in_percentage }}"
                                            ...
                                            <div
                                                ...
                                                <div class="relative flex items-stretch w-full">
                                                    <x-base.form-input class="form-control" id="validation-form-1"
                                                        ...
                                                        value="{{ $Order->discount_in_percentage }}" />
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <table
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td class="text-gray-900 text-end pe-0">
                                                                <input class="form-control sub_total" name="sub_total"
                                                                    type="hidden" value="{{ $Order->sub_total ?? 0 }}"
                                                                    onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))"
                                                                    min="0"
                                                                    onkeypress="if(this.value.length==8) return false;"
                                                                    minlength="0" required readonly
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-gray-900 text-end pe-0">
                                                                <span>&#8377;</span> <span
                                                                    id="quoteDiscountAmount">{{ $Order->discount ?? 0 }}</span>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td class="ps-0">TAX:</td>
                                                            <input class="form-control t_tax" name="t_tax"
                                                                type="hidden" value="{{ $Order->total_text ?? 0 }}"
                                                                onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))"
                                                                min="0"
                                                                onkeypress="if(this.value.length==8) return false;"
                                                                minlength="0" required readonly
                                                                value="{{ $Order->total_text ?? 0 }}" />
                                                            <td class="text-gray-900 text-end pe-0">
                                                                <span>&#8377;</span> <span
                                                                    id="tax">{{ $Order->total_text ?? 0 }}</span>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            ...
                                                            <td class="text-gray-900 text-end pe-0">
                                                                <input class="form-control final_amount"
                                                                    ...
                                                                    value="{{ $Order->total ?? 0 }}" />
                                                                <span>&#8377;</span> <span
                                                                    id="quoteFinalAmount">{{ $Order->total ?? 0 }}</span>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
...
@section('js')
    ...
    <script>
        $("#payment_type").on('change', function() {
            if (this.value === '1') {
                $('.check_number').show();
                $('.rtgs_number').hide(); // Hide rtgs_number when payment_type is 1
                $('.bank_id').show();
            } else if (this.value === '2') {
                $('.check_number').hide(); // Hide check_number when payment_type is 2
                $('.rtgs_number').show();
                $('.bank_id').show();
            } else {
                $('.check_number').hide();
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            function calculateRowAmounts(row) {
                ...
                var gstAmount = subtotal * (totalGST / 100);

                total = subtotal + gstAmount;

                row.querySelector('.taxes').value = gstAmount.toFixed(0);
                console.log(subtotal);
                row.querySelector('.tbAmount').value = total.toFixed(0);
            ...
            if (event.target && event.target.id === 'product') {
                ...
                jQuery.ajax({
                    ...
                    success: function(data) {
                        ...
                        var quantityInput = selectedRow.querySelector(
                            '.table__qty input[type="text"]');
                        ...
                        var unitLabel = selectedRow.querySelector(
                            '.table__qty .input-group-volume');
                        ...
                        if (decimalValue == 0) {
                            $('#quoteDiscountAmount').text(discount.toFixed(0));
                            $('#quoteFinalAmount').text((quoteTotal - discount + tax).toFixed(
                                0));
