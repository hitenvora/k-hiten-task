...
@section('subcontent')
    <div class="intro-y mt-8 flex items-center">
        <h2 class="mr-auto text-lg font-medium">Edit Entry</h2>
    </div>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form class="customers-validate-form" action="{{ route('entry.update', $Entry->id) }}" method="POST"
                                        ...
                                        <table
                                            ...
                                            <thead>
                                                <tr class="border-b fs-7 fw-bolder text-gray-700 uppercase">
                                                    {{-- <th scope="col">#</th> --}}
                                                    <th scope="col" class="required">Purcahse Id</th>
                                            ...
                                            <tbody class="quote-item-container table-body">
                                                <tr class="border-b border-bottom-dashed tax-tr">
                                                    <td class="table__qty">
                                                        <select aria-label=".form-select-lg example" id="perch_id"
                                                            ...
                                                            @foreach ($purchse_id as $item)
                                                                <option
                                                                    value="{{ $item->id }} "@if ($Entry->perch_id == $item->id) selected @endif>
                                                                    O-ID: {{ $item->id }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <td>
                                                        <select aria-label=".form-select-lg example" id="rupee"
                                                            class="disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&amp;[readonly]]:bg-slate-100 [&amp;[readonly]]:cursor-not-allowed [&amp;[readonly]]:dark:bg-darkmode-800/50 transition duration-200 ease-in-out w-full border-slate-200 shadow-sm rounded-md px-3 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 text-lg py-1.5 pl-4 pr-8 sm:mt-2 sm:mr-2"
                                                            name="rupee" style="margin: 0;" required>
                                                            @foreach ($purchse_id as $item)
                                                                <option value="{{ $item->id }}"
                                                                    @if ($Entry->rupee == $item->total) selected @endif>
                                                                    {{ $item->total }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <td>
                                                        <select aria-label=".form-select-lg example" id="credit_debit"
                                                            class="disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&amp;[readonly]]S:bg-slate-100 [&amp;[readonly]]:cursor-not-allowed [&amp;[readonly]]:dark:bg-darkmode-800/50 transition duration-200 ease-in-out w-full border-slate-200 shadow-sm rounded-md px-3 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 text-lg py-1.5 pl-4 pr-8 sm:mt-2 sm:mr-2"
                                                            name="credit_debit" style="margin: 0;" required>
                                                            <option value="">Please Select Credit/Debit</option>
                                                            <option value="0"
                                                                {{ $Entry->credit_debit == 0 ? 'selected' : '' }}>Credit
                                                            </option>
                                                            <option value="1"
                                                                {{ $Entry->credit_debit == 1 ? 'selected' : '' }}>Debit
                                                            </option>
                                                        </select>
                                                    ...
                                                    <td>
                                                        <x-base.input-group class="" inputGroup>
                                                            <x-base.form-input type="text" class="narration"
                                                                ...
                                                                value="{{ $Entry->narration }}" required />
                                        ...
                                        <div
                                            ...
                                            <div
                                                ...
                                                <select aria-label=".form-select-lg example" id="payment_type"
                                                    ...
                                                    <option value="0"
                                                        @if ($Entry->payment_type == '0') selected @endif>Cash</option>
                                                    <option value="1"
                                                        @if ($Entry->payment_type == '1') selected @endif>Check</option>
                                                    <option value="2"
                                                        @if ($Entry->payment_type == '2') selected @endif>RTGS</option>
                                                </select>
                                                <div class="check_no"
                                                    @if ($Entry->payment_type != '1') style="display: none;" @endif>
                                                    ...
                                                    <div class="relative flex items-stretch w-full">
                                                        <x-base.form-input class="form-control" name="check_no"
                                                            ...
                                                            onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))"
                                                            pattern="^\d*(\.\d{0,2})?$" name="check_no" minlength="1"
                                                            value="{{ $Entry->check_no }}" required />
                                                    </div>
                                                </div>
                                                <div class="rtgs_no"
                                                    @if ($Entry->payment_type != '2') style="display: none;" @endif>
                                                    <label for="discount" class="form-label mb-1">RTGS Number</label>
                                                    <div class="relative flex items-stretch w-full">
                                                        <x-base.form-input class="form-control" name="rtgs_no"
                                                            id="rtgs_no" type="text" placeholder="RTGS Number"
                                                            onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))"
                                                            value="{{ $Entry->check_no }}" pattern="^\d*(\.\d{0,2})?$"
                                                            name="rtgs_no" minlength="1" required />
                                                    </div>
                                                </div>
                                            </div>
                                            {{-- <div
                                                ...
                                                <div class="relative flex items-stretch w-full">
                                                    <x-base.form-input class="form-control" id="validation-form-1"
                                                        ...
                                                        name="discount" minlength="1" required />
                                                </div>
                                            </div> --}}
                                        </div>
