...
@section('subcontent')
    ...
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form class="customers-validate-form" action="{{ route('update.amin.order', $Order->id) }}" method="POST"
                                ...
                                <select data-tw-merge aria-label=".form-select-lg example" id="Customer"
                                    ...
                                    @foreach ($AdminCustomer as $data)
                                        <option value="{{ $data->id }}"
                                            @if ($Order->admin_client_id == $data->id) selected @endif>{{ $data->first_name }}
                                            {{ $data->last_name }}
                                        </option>
                                    @endforeach
                                </select>
                                                ...
                                                @if ($Order->GetAdminCart->count() != 0)
                                                    @foreach ($Order->GetAdminCart as $item)
                                                        <tr class="border-b border-bottom-dashed tax-tr">
                                                            <td class="table__item-desc w-1/4">
                                                                <select data-tw-merge aria-label=".form-select-lg example"
                                                                    ...
                                                                    @foreach ($Product as $data)
                                                                        ...
                                                                        @elseif($item->product_id == $data->id)
                                                                            <option value="{{ $data->id }}"
                                                                                @if ($item->product_id == $data->id) selected @endif>
                                                                                {{ $data->product_name }}
                                                                            </option>
                                                            ...
                                                            <td class="table__qty">
                                                                <x-base.input-group class="" inputGroup>
                                                                    <x-base.form-input type="text" class="quantity"
                                                                        ...
                                                                        {{-- max='{{ $item->product_quntity + ($item->Product->GetInventory ? $item->Product->GetInventory->inventorie : 0) }}' --}} min='0' />
                                                            ...
                                                            <td>
                                                                @php
                                                                    $tbAmount = $item->sub_total + $item->taxes;
                                                                @endphp
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td class="text-gray-900 text-end pe-0">
                                                                ...
                                                                <span>â‚¹</span> <span id="quoteTotal"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            function calculateRowAmounts(row) {
                ...
                tbAmount = subtotal.toFixed(3) + totalGST.toFixed(3);
                const gstAmount = totalGST * quantity;
                // total = subtotal + gstAmount;
                row.querySelector('.taxes').value = gstAmount.toFixed(2);
                row.querySelector('.tbAmount').value = tbAmount.toFixed(2);
            ...
            function updateQuoteTotal() {
                ...
                if (typeof variable === 'undefined' || decimalValue !== 0) {
